...
in the Roda application, then use the sessions plugin:
  ...
  require 'roda'
  class App < Roda
    plugin :sessions, cipher_secret: ENV['CIPHER_SECRET'],
                      hmac_secret: ENV['HMAC_SECRET']
  end

The +:cipher_secret+ and +:hmac_secret+ options should be distinct 32-byte randomly
generated strings.
...
that ships with Roda:
  ...
  class App < Roda
    use RodaSessionMiddleware, cipher_secret: ENV['CIPHER_SECRET'],
                               hmac_secret: ENV['HMAC_SECRET']
  end
...
For any cookie-based sessions, make sure that the necessary secrets (+:cipher_secret+
and +:hmac_secret+ when using Roda's session support, or +:secret+ when using
<tt>Rack::Session::Cookie</tt>) are not disclosed to an attacker.  Knowledge of the
