...
So a simple routing tree may look something like this (slightly compressed):

  r.on "a" do        # /a branch
    r.on "b" do      # /a/b branch
      r.is "c" do    # /a/b/c request
        r.get do     # GET /a/b/c request
        r.post do    # POST /a/b/c request
      r.get "d" do   # GET /a/b/d request
      r.post "e" do  # POST /a/b/e request

It's also possible to handling the same requests, but structure the
routing tree by first branching on the request method:

  r.get do           # GET 
    r.on "a"         # GET /a branch
      r.on "b"       # GET /a/b branch
        r.is "c"     # GET /a/b/c request
        r.is "d"     # GET /a/b/d request
  r.post do          # POST
    r.on "a"         # POST /a branch
      r.on "b"       # POST /a/b branch
        r.is "c"     # POST /a/b/c request
        r.is "e"     # POST /a/b/e request
...
matchers.  If all of the matchers match, the match block is yielded
to.  Here's an example showcasing how different matchers work.
...
When you use +r.run+, it is going to call the given Rack app (+API+ in this
...
This allows you so set instance variables in the main route block, and still
